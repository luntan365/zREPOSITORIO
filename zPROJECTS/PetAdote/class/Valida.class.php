...
class Valida {
  ...
  public static function texto($text, $sizemax=45, $sizemin=1)  {
    $text = trim(strip_tags($text));
    if (empty($text) && $sizemin > 0):
  ...
  public static function numero_inteiro($numero, $sizemax=10, $sizemin=1){
    $numero = self::texto($numero, $sizemax, $sizemin);
    if (!preg_match('/^[0-9]*$/', $numero) && $sizemin > 0)
      throw new Exception("Número inválido.");
    return $numero;
  }

  public static function email($email, $sizemax=100, $sizemin=1)  {
    $email = self::texto($email, $sizemax, $sizemin);
