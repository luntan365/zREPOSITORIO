...
<form class="formulario_cadastro" action="" method="post" autocomplete="off" enctype="multipart/form-data">
  ...
  <div class="row">
    ...
    <div class="input-field col s3 offset-s1">
      <input id="cd_numero" name="numero" type="text" required tabindex="7" maxlength="10">
      <label for="cd_numero">Número</label>
    </div>

    <div class="input-field col s3">
      <input id="cd_valor" name="valor" type="number"  min="0" required step="0.01"  tabindex="8" oninput="if(this.value.length>10) this.value = this.value.slice(0, 10);" >
  ...
  <div class="row">
    <div class="input-field col s5">
      <input id="cd_bairro" name="bairro" type="text" maxlength="200" required tabindex="2" value="<?php if (isset($_SESSION['cadastro']['bairro'])) { echo $_SESSION['cadastro']['bairro']; } ?>">
      <label for="cd_bairro">Bairro</label>
    </div>

    <div class="input-field col s6 offset-s1">
      <input id="cd_descricao" name="descricao" type="text" maxlength="2000"  tabindex="9">
  ...
  <div class="row">
    ...
    <div class="input-field col s5">
      <input id="cd_referencia" name="referencia" type="text" maxlength="200" tabindex="3" value="<?php if (isset($_SESSION['cadastro']['referencia'])) { echo $_SESSION['cadastro']['referencia']; } ?>">
      <label for="cd_referencia">Referência</label>
    </div>

    <div class="input-field col s2 offset-s1">
      <input id="cd_quarto" name="quarto" type="number" min="0" tabindex="10" oninput="if(this.value.length>3) this.value = this.value.slice(0, 3);">
      <label for="cd_quarto">Quarto</label>
    </div>

    <div class="input-field col s2">
      <input id="cd_suite" name="suite" type="number" min="0"  tabindex="11" oninput="if(this.value.length>3) this.value = this.value.slice(0, 3);">
      <label for="cd_suite">Suite</label>
    </div>

    <div class="input-field col s2">
      <input id="cd_banheiro" name="banheiro" type="number" min="0" tabindex="12" oninput="if(this.value.length>3) this.value = this.value.slice(0, 3);">
  ...
  <div class="row">
    <div class="col s5">
      <p>
        <input type="radio" name="categoria" value="casa" class="with-gap" tabindex="4" id="cd_casa" required <?php if (isset($_SESSION['cadastro']['categoria']) && $_SESSION['cadastro']['categoria']=="casa") { echo "checked"; } ?> />
        <label for="cd_casa">Casa</label>
      </p>
      <p>
        <input type="radio" name="categoria" value="ap" class="with-gap" tabindex="5" id="cd_ap" required <?php if (isset($_SESSION['cadastro']['categoria']) && $_SESSION['cadastro']['categoria']=="ap") { echo "checked"; } ?>/>
        <label for="cd_ap">Apartamento</label>
      </p>
      <p>
        <input type="radio" name="categoria" value="comercial" class="with-gap" tabindex="6" id="cd_comercial" required <?php if (isset($_SESSION['cadastro']['categoria']) && $_SESSION['cadastro']['categoria']=="comercial") { echo "checked"; } ?>/>
        <label for="cd_comercial">Ponto Comercial</label>
      </p>
    </div>

    <div class="input-field col s2 offset-s1">
      <input id="cd_garagem" name="garagem" type="number" min="0"  tabindex="13" oninput="if(this.value.length>3) this.value = this.value.slice(0, 3);">
      <label for="cd_garagem">Capacidade Garagem</label>
    </div>

    <div class="col s2 caixa_check">
      <input type="checkbox" name="quintal" class="filled-in" tabindex="14" id="cd_quintal"/>
      <label for="cd_quintal">Quintal</label>
    </div>

    <div class="col s2 caixa_check">
      <input type="checkbox" name="varanda" class="filled-in" tabindex="15" id="cd_varanda"/>
      <label for="cd_varanda">Varanda</label>
    </div>

    <div class="col s2 caixa_check offset-s1">
      <input type="checkbox" name="status" class="filled-in"  tabindex="16" id="cd_disponivel" checked/>
      <label for="cd_disponivel">Disponível</label>
    </div>

    <div class="input-field col s1 offset-s1" id="botao-adicionar">
      <button class="btn waves-effect waves-light" type="submit"  tabindex="17" name="adicionar" formaction="cadastro_adicionar">Adicionar</button>
    </div>
